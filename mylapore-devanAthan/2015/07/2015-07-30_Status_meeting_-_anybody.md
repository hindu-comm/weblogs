+++
title = "Status meeting anybody ?"
full_title = "Status meeting anybody ?"
upstream_url = "https://mylapore.substack.com/p/status-meeting-anybody"
date = "2015-07-30"
+++
Source: [here](https://mylapore.substack.com/p/status-meeting-anybody).

Status meeting - anybody ?

# Status meeting - anybody ?

[](https://substack.com/@amaruvi)

![Amaruvi Devanathan's avatar](https://substackcdn.com/image/fetch/$s_!Bq_i!,w_36,h_36,c_fill,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F357700fd-8e69-44fa-a5f8-000e6e26808a_1002x1002.jpeg)

[Amaruvi Devanathan](https://substack.com/@amaruvi)

Jul 30, 2015

![](data:image/svg+xml;base64,)

![](data:image/svg+xml;base64,)

![](data:image/svg+xml;base64,)

Share

If you want tokill time, kill productivity but stillfill time and pretend to work, then attend a status meeting.

Ask any corporate executive or IT services worker, he would say on oath that the one event that does not have a specific start and end time but is held regularly to know the status of an activity, is called Status Meeting.

This meeting's only agenda is to discuss the status of an activity - a project, a task and the like. Buteverything else other than status gets discussed.

What happens in a status meeting ? Generally a group of ten people gather to discuss on something that never happened all the while pretending that the thing has happened. And that is what happens.

Butyou had gathered to know the status, right ? So, as you didn't discuss status, you didn't know the status. So what do you do to know the status ?

Call for a follow-up status meeting.

Andwhat happens in a follow-up status meeting ?

Go to line 1.

![](data:image/svg+xml;base64,)

![](data:image/svg+xml;base64,)

![](data:image/svg+xml;base64,)

Share
